...
export function startExtensionWatcher(options: WatcherOptions): FSWatcher[] {
  ...
  const triggerReload = async (source: string) => {
    ...
    try {
      console.error(`ray-ai-tools: Detected change in ${source}, reloading...`);
      const changed = await onReload();
      if (changed) {
        const servers = getServers();
        console.error(
          `ray-ai-tools: Tools changed, notifying ${servers.length} client(s)`
        );
        for (const server of servers) {
          try {
            ...
            console.error(`ray-ai-tools: Failed to notify client: ${err.message}`);
          }
        }
      }
    } catch (err: any) {
      console.error(`ray-ai-tools: Reload failed: ${err.message}`);
    } finally {
      isReloading = false;
    }
  };
  ...
  try {
    ...
    console.error(`ray-ai-tools: Watching ${extensionsDir}`);
  } catch (err: any) {
    console.error(`ray-ai-tools: Could not watch extensions dir: ${err.message}`);
  }
  ...
  try {
    ...
    console.error(`ray-ai-tools: Watching ${raycastSupportDir}`);
  } catch (err: any) {
    console.error(`ray-ai-tools: Could not watch Raycast dir: ${err.message}`);
  }
  ...
  try {
    ...
    console.error(`ray-ai-tools: Watching ${raybridgeConfigDir}`);
  } catch (err: any) {
  ...
  try {
    ...
    console.error(`ray-ai-tools: Watching ${rayAiToolsConfigDir}`);
  } catch (err: any) {
    // Config dir might not exist
  }
