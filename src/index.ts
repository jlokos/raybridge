...
export function createMcpServer(ctx: ServerContext): Server {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    console.error(`ray-ai-tools: [CALL] ${extName}/${args.tool_name} input=${inputSummary}`);
...
export async function loadServerContext(): Promise<ServerContext> {
  ...
  try {
    ...
    if (prefsCount > 0) {
      console.error(
        `ray-ai-tools: Loaded preferences for ${prefsCount} extensions from Raycast DB`
      );
    }
  } catch (err: any) {
    console.error(`ray-ai-tools: Could not load Raycast preferences: ${err.message}`);
  }
  ...
  try {
    ...
    console.error(
      `ray-ai-tools: Loaded OAuth tokens for ${raycastTokens.size} extensions`
    );
  } catch (err: any) {
    console.error(`ray-ai-tools: Could not load OAuth tokens: ${err.message}`);
  }
  ...
  if (extensions.length < localExtensions.length) {
    ...
    console.error(`ray-ai-tools: ${disabled} extension(s) disabled by config`);
  }
  ...
  console.error(
    `ray-ai-tools: Registered ${extensions.length} extensions (${toolCount} tools total)`
  );
...
export async function reloadServerContext(ctx: ServerContext): Promise<boolean> {
  ...
  try {
    ...
    if (prefsCount > 0) {
      console.error(
        `ray-ai-tools: Reloaded preferences for ${prefsCount} extensions`
      );
    }
  } catch (err: any) {
    console.error(`ray-ai-tools: Could not reload Raycast preferences: ${err.message}`);
  }
  setPreferences(mergedPrefs);
  ...
  try {
    ...
    setRaycastTokens(raycastTokens);
    console.error(
      `ray-ai-tools: Reloaded OAuth tokens for ${raycastTokens.size} extensions`
    );
  } catch (err: any) {
    console.error(`ray-ai-tools: Could not reload OAuth tokens: ${err.message}`);
  }
  ...
  const toolCount = extensions.reduce((n, e) => n + e.tools.length, 0);
  console.error(
    `ray-ai-tools: Reloaded ${extensions.length} extensions (${toolCount} tools total)`
  );
