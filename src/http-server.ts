...
export async function startHttpServer(options: HttpServerOptions): Promise<void> {
  ...
  app.all("/mcp", async (req: Request, res: Response) => {
    ...
    if (!sessionId && req.method === "POST" && isInitializeRequest(req.body)) {
      const transport = new StreamableHTTPServerTransport({
        ...
        onsessioninitialized: (id: string) => {
          ...
          console.error(`ray-ai-tools: HTTP session started: ${id}`);
        },
      });
    ...
    if (sessionId && req.method === "DELETE") {
      ...
      if (session) {
        ...
        console.error(`ray-ai-tools: HTTP session terminated: ${sessionId}`);
  ...
  const cleanupInterval = setInterval(() => {
    ...
    for (const [id, session] of sessions) {
      if (now - session.lastActivity > IDLE_TIMEOUT) {
        ...
        sessions.delete(id);
        console.error(`ray-ai-tools: HTTP session expired: ${id}`);
      }
    }
  ...
  process.on("SIGINT", async () => {
    console.error("\nray-ai-tools: Shutting down HTTP server...");
    clearInterval(cleanupInterval);
    for (const [id, session] of sessions) {
      onServerClosed?.(session.server);
      await session.server.close();
      console.error(`ray-ai-tools: Closed session: ${id}`);
    }
    process.exit(0);
  });
  ...
  app.listen(port, host, () => {
    console.error(`ray-ai-tools: HTTP server listening on http://${host}:${port}`);
    console.error(`ray-ai-tools: MCP endpoint: http://${host}:${port}/mcp`);
    console.error(`ray-ai-tools: Health check: http://${host}:${port}/health`);
    if (apiKey) {
      console.error("ray-ai-tools: API key authentication enabled");
    }
  });
}
